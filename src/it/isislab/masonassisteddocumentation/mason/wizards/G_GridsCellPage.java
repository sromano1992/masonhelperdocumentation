...
public class G_GridsCellPage extends WizardPage{
	...
	private void addToGroup(Parameter p) {
		if (!(p.getJavadoc().contains("@ingroup entities"))) //$NON-NLS-1$
			GlobalUtility.setJavadocToField(simStateAnalizer.getRoot(), p.getField(), p.getJavadoc() + GlobalUtility.COMMENT_SIGNATURE + " @ingroup entities"); //$NON-NLS-1$
	...
	public IWizardPage getNextPage(){ 
		...
		for (int i=0; i<fields.size(); i++){
			...
			String commentValue = GlobalUtility.COMMENT_SIGNATURE +  "@ingroup entities\n *\n"; //$NON-NLS-1$
			commentValue = commentValue + GlobalUtility.surroundWithSpan(GlobalUtility.userOutputColor, userComment) + "\n"; //$NON-NLS-1$
			commentValue = commentValue + GlobalUtility.surroundWithSpan(GlobalUtility.autoOutputColor, autogeneratedComment);
