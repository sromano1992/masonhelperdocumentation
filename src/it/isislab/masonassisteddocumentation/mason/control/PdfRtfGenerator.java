...
public class PdfRtfGenerator {
		...
		try {
			...
			footer.setBorder(Rectangle.NO_BORDER);
			            ...
			            HeaderFooter header = new HeaderFooter(
			                        new Phrase(GlobalUtility.MAD_SIGNATURE, new Font(getFont(), 7, Font.NORMAL)), false);
	...
	private void createElement(Document document, Set<Entry<String, String>> element){
		Paragraph newPar = null, parContent = null;
		...
		for (Entry<String, String> e : element) {
			...
			if (cycle == 0){	//title!
				if (e.getKey().contains("_subTitle")){
					...
					parContent = new Paragraph(e.getValue(), parLine);
					newPar.add(content);
					//newPar.add(content);
					firstElementValue = e.getValue();
					try {
						document.add(newPar);
						document.add(parContent);
