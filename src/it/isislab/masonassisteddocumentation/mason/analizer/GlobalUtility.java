...
import it.isislab.masonassisteddocumentation.visitor.JavaDocVisitor;
import it.isislab.masonassisteddocumentation.visitor.StartStepMethodVisitor;
...
public class GlobalUtility {
	...
	public static String COMMENT_SIGNATURE = "<!-- MAD(MASON ASSISTED DOCUMENTATION) AUTOGENERATED COMMENT, built by Simone Romano in ISISLab -->\n";
	public static String MAD_SIGNATURE = "Document has been generated by MAD(Mason assisted documentation), built by Simone Romano in ISISLab. \n";
	public static String documentDescription = "This documentation contains information on the simulation model. These information are partially self-generated and partially entered.\n"		
											 ...
											 + "<h3>Entities, state variables and scales</h3> What kinds of entities are in the model? By what state variables, or attributes, are these entities characterized?\n<br>"
	...
	private void checkFooterPath() {
		...
		if (footer.exists())
			footerPath = footer.getAbsolutePath();
	...
	public static String getStartMethodInformation(Method start){
		StartStepMethodVisitor cv = new StartStepMethodVisitor(simStateAnalizer.getCompilationUnit());	//start method is in simState
	...
	public static String getStepMethodInformation(Method step){
		StartStepMethodVisitor sv = new StartStepMethodVisitor(getAgentAnalizer().getCompilationUnit());
		if (step != null){
	...
	/**
	 * This method remove comment autogenerated by MAD
	 * from SimState and Agents.
	 */
	public static void removeMADComment(boolean saveBackup) {
		if (saveBackup)
			saveSourceBackupCopy();
		//SimState
		JavaDocVisitor jdv = new JavaDocVisitor(simStateAnalizer.getCompilationUnit());
		simStateAnalizer.getCompilationUnit().accept(jdv);
		simStateAnalizer.writeSourceFile();
		log.info("Removed comment from SimState: " + simStateAnalizer.getClassName());
		
		//Agent_s
		for (AgentAnalizer agent : agent_sAnalizer){
			jdv = new JavaDocVisitor(agent.getCompilationUnit());
			agent.getCompilationUnit().accept(jdv);
			agent.rewrite();
			log.info("Removed comment from agent: " + agent.getClassName());
		}
	}

	private static void saveSourceBackupCopy() {
		simStateAnalizer.writeSoruceFile(ConfigFile.getSourceBackupDir() + File.separator + simStateAnalizer.getClassName() + "._MAD");
		for (AgentAnalizer agent : agent_sAnalizer){
			agent.rewrite(ConfigFile.getSourceBackupDir() + File.separator + agent.getClassName() + "._MAD");
		}
		log.info("Saved code copy with doxygen comments");
	}
