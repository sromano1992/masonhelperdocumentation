...
public class F_AgentVariablesPage extends WizardPage{
	...
	private static String pageDescription = "Here information about other variables:";
	...
	public void myCreateControl(){
		...
		ArrayList<Parameter> allParameters = GlobalUtility.getAllParameters(agentAnalizer.getCompilationUnit());
		for (Parameter p : allParameters)
			if (!(agentAnalizer.alreadyVisited(p)))	remainingParameters.add(p);	
		...
		for (Parameter p : remainingParameters){
			...
			if (p.getInitializer()!=""){
				lblInitializerDetector.setText("Initial value of " + p.getVariableName() + " variable is: \n" + p.getInitializer());
			}
	...
	public IWizardPage getNextPage(){ 
		...
		for (int i=0; i<inputText.size(); i++){
			if (!(autoGeneratedComment.get(i).getText().equals("")) || !((inputText.get(i).getText().equals("")))){	//if there is at least an autogenerated comment
				String comment = "@ingroup entities\n*\n" + inputText.get(i).getText() + "\n" + autoGeneratedComment.get(i).getText();
