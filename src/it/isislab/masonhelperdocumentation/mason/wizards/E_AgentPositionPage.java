...
public class E_AgentPositionPage extends WizardPage{	
	...
	private String pageDescription = "Here information about agent's position.";
	...
	public void createControl(Composite parent) {
		...
		if (positions != null && positions.size()!=0){
			...
			for (int i=0; i<positions.size(); i++){			
				...
				lblPositionName.setText(positions.get(i).getVariableType() + " " + positions.get(i).getVariableName());	
				lblInitializerDetector.setText("Variable '" + positions.get(i).getVariableType() + " " 
										+ positions.get(i).getVariableName() +"' contains information "
										+ "about agent's position.");	
				if (positions.get(i).getInitializer() != "")	
					lblInitializerDetector.setText(lblInitializerDetector.getText() + "\n Initial value of variable is: " + positions.get(i).getInitializer());
	...
	public IWizardPage getNextPage(){ 
		...
		for (int i=0; i<positions.size(); i++){
			String autoGeneratedComment = positionAutomaticDescription.get(i).getText();
			String userComment = positionUserDescription.get(i).getText();
			...
			ODD.addVariableToEntity(agentAnalizer.getClassName(), new Variable(positions.get(i).getVariableName(), userComment, autoGeneratedComment, positions.get(i).getInitializer()));
